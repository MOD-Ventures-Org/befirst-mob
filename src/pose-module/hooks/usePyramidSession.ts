 ...
 * - COMPLETE: terminal session outcome.
 *
 * There is intentionally no inactivity timeout. Athletes can take as long as
 * needed to finish a set; only the explicit exit control ends a workout early.
 */
...
const ACTIVE_PHASES: PyramidPhase[] = ['SET_ACTIVE', 'SET_DONE', 'REST', 'CELEBRATE'];
...
export interface PyramidSessionState {
	...
	// Visible countdown number for REST, else null.
 ...
 * set sequence and rest countdowns on top.
 *
 * Per-rep events feed the state machine through `usePoseSession`'s `onRep`
	 * callback; a lightweight ticker drives the time-based transitions. Counting
	 * correctness is owned by the existing engine and is intentionally out of
	 * scope for this POC.
 */
export function usePyramidSession() {
	...
	const { pose, signal, coachState, debugInfo, initError, androidPerformanceTier, solution, hasPermission, requestPermission, start, stop } =
		usePoseSession({
	...
	const handleRep = useCallback(() => {
		...
		if (current === 'READY' || current === 'CELEBRATE' || current === 'COMPLETE') return;
